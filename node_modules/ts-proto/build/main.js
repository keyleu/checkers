  ...
  `);
    ...
    messageDesc.field.forEach((field) => {
                ...
                if (types_1.isRepeated(field) && types_1.isMapType(ctx, messageDesc, field)) {
                    ...
                    if (types_1.isPrimitive(valueField)) {
                        ...
                        if (types_1.isBytes(valueField)) {
                            ...
                            return ts_poet_1.code `${from} as ${valueType}`;
                        }
                        else {
                    ...
                    else if (types_1.isObjectId(valueField) && options.useMongoObjectId) {
                        return ts_poet_1.code `${utils.fromJsonObjectId}(${from})`;
                    }
                    else if (types_1.isTimestamp(valueField) && options.useDate === options_1.DateOption.STRING) {
                        return ts_poet_1.code `String(${from})`;
                    }
                    else if (types_1.isTimestamp(valueField) &&
                        (options.useDate === options_1.DateOption.DATE || options.useDate === options_1.DateOption.TIMESTAMP)) {
                        return ts_poet_1.code `${utils.fromJsonTimestamp}(${from})`;
                    }
                    else if (types_1.isValueType(ctx, valueField)) {
                        return ts_poet_1.code `${from} as ${valueType}`;
                    }
                    else if (types_1.isAnyValueType(valueField)) {
                        return ts_poet_1.code `${from}`;
                    }
                    else {
                        return ts_poet_1.code `${valueType}.fromJSON(${from})`;
                    }
                }
                else {
                    const type = types_1.basicTypeName(ctx, field);
        ...
        if ((_a = canonicalFromJson[fullTypeName]) === null || _a === void 0 ? void 0 : _a[fieldName]) {
            chunks.push(ts_poet_1.code `${fieldName}: ${canonicalFromJson[fullTypeName][fieldName]('object')},`);
        }
        ...
        `);
                ...
                if (readValueSnippet.toString() === ts_poet_1.code `e`.toString()) {
                    chunks.push(ts_poet_1.code `${fieldName}: Array.isArray(${jsonPropertyOptional}) ? [...${jsonProperty}] : [],`);
                }
                else {
          ...
          `);
            ...
            const ternaryIf = ts_poet_1.code `${ctx.utils.isSet}(${jsonProperty})`;
            const ternaryThen = ts_poet_1.code `{ $case: '${fieldName}', ${fieldName}: ${readSnippet(`${jsonProperty}`)}`;
            ...
            chunks.push(ts_poet_1.code `${fieldName}: ${ctx.utils.isSet}(${jsonPropertyOptional})
        ? ${readSnippet(`${jsonProperty}`)}
        : undefined,
      `);
        }
        else if (types_1.isStructType(field)) {
            chunks.push(ts_poet_1.code `${fieldName}: ${ctx.utils.isObject}(${jsonProperty})
          ? ${readSnippet(`${jsonProperty}`)}
          : undefined,`);
        }
        else if (types_1.isListValueType(field)) {
...
function generateCanonicalToJson(fullName, fullProtobufTypeName) {
    if (types_1.isFieldMaskTypeName(fullProtobufTypeName)) {
        return ts_poet_1.code `
...
function generateToJson(ctx, fullName, fullProtobufTypeName, messageDesc) {
  ...
  `);
    ...
    messageDesc.field.forEach((field) => {
        const fieldName = case_1.maybeSnakeToCamel(field.name, options);
        const jsonName = utils_1.getFieldJsonName(field, options);
        const jsonProperty = utils_1.getPropertyAccessor('obj', jsonName);
        const readSnippet = (from) => {
            if (types_1.isEnum(field)) {
                ...
                const type = types_1.basicTypeName(ctx, field, { keepValueType: true });
                return ts_poet_1.code `${type}.toJSON(${type}.wrap(${from}))`;
            }
      ...
      `);
            ...
            chunks.push(ts_poet_1.code `message.${oneofName}?.$case === '${fieldName}' && (${jsonProperty} = ${v});`);
        }
        else {
            const v = readSnippet(`message.${fieldName}`);
            chunks.push(ts_poet_1.code `message.${fieldName} !== undefined && (${jsonProperty} = ${v});`);
        }
    });
            ...
            if ((types_1.isLong(field) || types_1.isLongValueType(field)) && options.forceLong === options_1.LongOption.LONG) {
                ...
                if (types_1.isRepeated(field) && types_1.isMapType(ctx, messageDesc, field)) {
                    ...
                    if (types_1.isPrimitive(valueField)) {
                        ...
                        const type = types_1.basicTypeName(ctx, valueField);
...
function generateWrap(ctx, fullProtoTypeName) {
    ...
    if (types_1.isFieldMaskTypeName(fullProtoTypeName)) {
        chunks.push(ts_poet_1.code `wrap(paths: string[]): FieldMask {
      return {paths: paths};
    }`);
    }
    return chunks;
...
function generateUnwrap(ctx, fullProtoTypeName) {
    ...
    if (types_1.isFieldMaskTypeName(fullProtoTypeName)) {
        chunks.push(ts_poet_1.code `unwrap(message: FieldMask): string[] {
